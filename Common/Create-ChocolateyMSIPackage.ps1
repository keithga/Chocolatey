...
if ( $x86uninstall -or $x64uninstall )
{
...
`$ChocoPackage = @{
$( if ( $x86uninstall -and -not $x64uninstall ) { "    SilentArgs = '$x86uninstall /quiet /norestart'" } )
    packageName = '$PackageName'
    fileType = 'msi'
    validExitCodes = @(0,3010)
}
...
{
    `$ChocoPackage.SilentArgs = '$x64uninstall /quiet /norestart'
}
else
{
    `$ChocoPackage.SilentArgs = '$x86uninstall /quiet /norestart'
}
